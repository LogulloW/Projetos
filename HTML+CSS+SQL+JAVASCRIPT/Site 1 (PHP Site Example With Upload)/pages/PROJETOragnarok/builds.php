<!DOCTYPE html>
<html>
  <head>
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <link rel="stylesheet" href="../../styles/stylesexta.css">
    <link href='//fonts.googleapis.com/css?family=Open+Sans:400,700' rel='stylesheet' type='text/css'>
    <script type="text/javascript" src="../js/jssext.js"></script>
    <link rel="icon" type="imagem/ico" href="../png/sextaico.png" />

  <title>~ Lá Mafia</title>
  
  </head>
  <body>
<section>

<nav id="menu">
    <ul>
       <div class="dropdown1">
        <li><a href="../sexta.php"><img src="../../png/wolfico.png" width="40" height="40">Início</a></li>
        <div class="dropdown-content1">
  <a href="../../index.php"><img src="../../png/wolfico.png" width="40" height="40">>Central<</a>
  </div>
  </div>


        <div class="dropdown1">
        <li><a href="builds.php"><img src="../../png/wolfico.png" width="40" height="40">Builds</a></li>
          <div class="dropdown-content1">
  <a href="espadachim.php"><img src="../../ragcon/1.png" >Espadachins</a>
  <a href="mago.php"><img src="../../ragcon/2.png">Magos</a>
  <a href="arqueiro.php"><img src="../../ragcon/3.png">Arqueiros</a>
  <a href="mercador.php"><img src="../../ragcon/4.png">Mercadores</a>
  <a href="gatuno.php"><img src="../../ragcon/5.png">Gatunos</a>
  <a href="novico.php"><img src="../../ragcon/6.png">Noviços</a>
  </div>
  </div>

          <div class="dropdown1">
        <li><a href="instancias.php"><img src="../../png/wolfico.png" width="40" height="40">Instâncias</a></li>
                  <div class="dropdown-content1">
  <a href="https://browiki.org/wiki/Mora" target="_blank"><img src="../../ragcon/Imagemdequest.png" width="25" height="25"><b><i>Mora LVL(100)</i></b></a>
  <a href="https://browiki.org/wiki/Glast_Heim" target="_blank"><img src="../../ragcon/Imagemdequest.png" width="25" height="25"><b><i>OGH LVL(130)</i></b></a>
  <a href="https://browiki.org/wiki/F%C3%A1brica_do_Terror" target="_blank"><img src="../../ragcon/Imagemdequest.png" width="25" height="25"><b><i>Celine LVL(140)</i></b></a>
  </div>
  </div>
       <div class="dropdown1">
        <li><a href="#"><img src="../../png/wolfico.png" width="40" height="40">Zeny</a></li>
         <div class="dropdown-content1">
  <a href="#"><img src="../../png/wolfico.png" width="40" height="40">Link 1</a>
  <a href="#"><img src="../../png/wolfico.png" width="40" height="40">Link 2</a>
  <a href="#"><img src="../../png/wolfico.png" width="40" height="40">Link 3</a>
  </div>
  </div>

         <div class="dropdown1">
        <li><a href="#"><img src="../../png/wolfico.png" width="40" height="40">Contato</a></li>
         <div class="dropdown-content1">
  <a href="#"><img src="../../png/wolfico.png" width="40" height="40">Link 1</a>
  <a href="#"><img src="../../png/wolfico.png" width="40" height="40">Link 2</a>
  <a href="#"><img src="../../png/wolfico.png" width="40" height="40">Link 3</a>
  </div>
  </div>

    </ul>
</nav>
</section>
<div  class="centersite">
  <h2 class="glow">>BUILDS<</h2><br>
  <h1>Este site foi feito com o propósito de proporcionar para os jogadores, builds e outras informações do jogo, as páginas de builds possuirão os Atributos, as Habilidades e os Items, além de informações do por que de cada escolha e algumas mecânicas das classes.<br>
    <form method="post" action="builds2.php">
      Valor:<input type=text name="v"/>
      <input type="submit" value="Calcular"/>
    </form>
    <br><br> Então, Divirtam-se!<br><br></h1><img class="centersite" src="../../jpg/Wallnarok.jpg">
</div>
  </body>
</html> 